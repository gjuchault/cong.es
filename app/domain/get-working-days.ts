...
const bankHolidays = new Set(rawBankHolidays.map(({ from }) => from.toString()));

export function getWorkingDays(
  { from, to, startHalf, stopHalf }:
    { from: Temporal.PlainDate; to: Temporal.PlainDate, startHalf: boolean, stopHalf: boolean },
): number {
  let workingDays = 0;
